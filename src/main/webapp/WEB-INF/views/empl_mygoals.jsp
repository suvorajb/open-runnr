	...
	<div class="container bootcards-container" id="main">
		...
		<div class="row">
			<div class="bs-example bs-example-type">
				<h2>My Enrolled Goals</h2>
				<div class="bootcards-list">
					<div class="panel panel-default">
						<c:if test="${not empty mygoals}">
							<div class="list-group">
								<c:forEach items="${mygoals}" var="mygoal">
									<a class="list-group-item"
										href="/open-runnr/employee/viewmygoal?enrolledgoalid=${mygoal.id}">
										<div class="row">
											<div class="col-sm-6">
												<i class="fa fa-3x fa-arrow-circle-o-right pull-left"></i>
												<h4 class="list-group-item-heading">${mygoal.goal_title}</h4>
												<p class="list-group-item-text">${mygoal.goal_desc}</p>
											</div>
											<div class="col-sm-6">
												<c:if test="${mygoal.goal_category eq 'Eating' }">
													<h3 class="list-group-item-text">
														<span class="label label-success">${mygoal.goal_category }</span>
													</h3>
												</c:if>
												<c:if test="${mygoal.goal_category eq 'Fitness' }">
													<h3 class="list-group-item-text">
														<span class="label label-warning">${mygoal.goal_category }</span>
													</h3>
												</c:if>
												<c:if test="${mygoal.goal_category eq 'LifeStyle' }">
													<h3 class="list-group-item-text">
														<span class="label label-primary">${mygoal.goal_category }</span>
													</h3>
												</c:if>
											</div>
										</div>
						...
						<c:if test="${empty mygoals}">

							<h3>You have not selected any Goals for you yet</h3>
